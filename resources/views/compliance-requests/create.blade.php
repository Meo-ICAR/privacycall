...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-4xl mx-auto">
        <div class="flex justify-between items-center mb-6">
            <div>
                <h1 class="text-3xl font-bold text-gray-900">Create Compliance Request</h1>
                <p class="text-gray-600 mt-2">Add a new incoming compliance request from a mandator</p>
            </div>
        ...
        <div class="bg-white rounded-lg shadow-lg">
            <form action="{{ route('compliance-requests.store') }}" method="POST" class="p-6">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <select name="mandator_id" id="mandator_id" required class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                            ...
                            @foreach($mandators as $mandator)
                                <option value="{{ $mandator->id }}" {{ old('mandator_id', $selectedMandatorId) == $mandator->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="request_type" id="request_type" required class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                            ...
                            <option value="compliance" {{ old('request_type') == 'compliance' ? 'selected' : '' }}>Compliance</option>
                            <option value="security" {{ old('request_type') == 'security' ? 'selected' : '' }}>Security</option>
                            <option value="gdpr" {{ old('request_type') == 'gdpr' ? 'selected' : '' }}>GDPR</option>
                            <option value="financial" {{ old('request_type') == 'financial' ? 'selected' : '' }}>Financial</option>
                            <option value="operational" {{ old('request_type') == 'operational' ? 'selected' : '' }}>Operational</option>
                            <option value="data_processing" {{ old('request_type') == 'data_processing' ? 'selected' : '' }}>Data Processing</option>
                        </select>
                        @error('request_type')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <select name="request_scope" id="request_scope" required class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                            ...
                            <option value="full" {{ old('request_scope') == 'full' ? 'selected' : '' }}>Full Audit</option>
                            <option value="partial" {{ old('request_scope') == 'partial' ? 'selected' : '' }}>Partial Audit</option>
                            <option value="specific_area" {{ old('request_scope') == 'specific_area' ? 'selected' : '' }}>Specific Area</option>
                            <option value="document_only" {{ old('request_scope') == 'document_only' ? 'selected' : '' }}>Document Only</option>
                        </select>
                        @error('request_scope')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="message" id="message" rows="6" required
                                  class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                                  placeholder="Enter the mandator's request message">{{ old('message') }}</textarea>
                        @error('message')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <select name="meeting_type" id="meeting_type" class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                            ...
                            <option value="document_review" {{ old('meeting_type') == 'document_review' ? 'selected' : '' }}>Document Review</option>
                        </select>
                        @error('meeting_type')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                ...
                <div class="flex justify-end space-x-3 mt-8 pt-6 border-t border-gray-200">
                    <a href="{{ route('compliance-requests.index') }}"
