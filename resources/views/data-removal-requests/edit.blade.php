<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Edit Data Removal Request') }} - {{ $dataRemovalRequest->request_number }}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('data-removal-requests.update', $dataRemovalRequest) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            <div>
                                ...
                                <select name="request_type" id="request_type" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                                    ...
                                    <option value="customer_direct" {{ old('request_type', $dataRemovalRequest->request_type) == 'customer_direct' ? 'selected' : '' }}>Customer Direct Request</option>
                                    <option value="mandator_request" {{ old('request_type', $dataRemovalRequest->request_type) == 'mandator_request' ? 'selected' : '' }}>Mandator Request</option>
                                    <option value="legal_obligation" {{ old('request_type', $dataRemovalRequest->request_type) == 'legal_obligation' ? 'selected' : '' }}>Legal Obligation</option>
                                    <option value="system_cleanup" {{ old('request_type', $dataRemovalRequest->request_type) == 'system_cleanup' ? 'selected' : '' }}>System Cleanup</option>
                                </select>
                                @error('request_type')
                            ...
                            <div>
                                ...
                                <select name="priority" id="priority" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                                    ...
                                    <option value="low" {{ old('priority', $dataRemovalRequest->priority) == 'low' ? 'selected' : '' }}>Low</option>
                                    <option value="medium" {{ old('priority', $dataRemovalRequest->priority) == 'medium' ? 'selected' : '' }}>Medium</option>
                                    <option value="high" {{ old('priority', $dataRemovalRequest->priority) == 'high' ? 'selected' : '' }}>High</option>
                                    <option value="urgent" {{ old('priority', $dataRemovalRequest->priority) == 'urgent' ? 'selected' : '' }}>Urgent</option>
                                </select>
                                @error('priority')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                                ...
                                <div>
                                    ...
                                    <select name="customer_id" id="customer_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                                        ...
                                        @foreach($customers as $customer)
                                            <option value="{{ $customer->id }}" {{ old('customer_id', $dataRemovalRequest->customer_id) == $customer->id ? 'selected' : '' }}>
                                ...
                                <div>
                                    ...
                                    <select name="mandator_id" id="mandator_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                                        ...
                                        @foreach($mandators as $mandator)
                                            <option value="{{ $mandator->id }}" {{ old('mandator_id', $dataRemovalRequest->mandator_id) == $mandator->id ? 'selected' : '' }}>
                                                {{ $mandator->full_name }} ({{ $mandator->email }})
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                        ...
                        <div class="mb-6">
                            ...
                            <textarea name="reason_for_removal" id="reason_for_removal" rows="4" required
                                      ...
                                      placeholder="Describe the reason for the data removal request...">{{ old('reason_for_removal', $dataRemovalRequest->reason_for_removal) }}</textarea>
                        ...
                        <div class="mb-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                                @php
                                    ...
                                    $selectedCategories = old('data_categories_to_remove', $dataRemovalRequest->data_categories_to_remove ?? []);
                                @endphp
                        ...
                        <div class="mb-6">
                            ...
                            <input type="date" name="due_date" id="due_date"
                                   value="{{ old('due_date', $dataRemovalRequest->due_date?->format('Y-m-d')) }}"
                                   class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                        ...
                        <div class="mb-6">
                            <div class="flex items-center">
                                <input type="checkbox" name="identity_verified" id="identity_verified" value="1"
                                       {{ old('identity_verified', $dataRemovalRequest->identity_verified) ? 'checked' : '' }}
                                       class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                            ...
                            <div class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <select name="verification_method" id="verification_method" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                                        ...
                                        <option value="email_verification" {{ old('verification_method', $dataRemovalRequest->verification_method) == 'email_verification' ? 'selected' : '' }}>Email Verification</option>
                                        <option value="phone_verification" {{ old('verification_method', $dataRemovalRequest->verification_method) == 'phone_verification' ? 'selected' : '' }}>Phone Verification</option>
                                        <option value="document_verification" {{ old('verification_method', $dataRemovalRequest->verification_method) == 'document_verification' ? 'selected' : '' }}>Document Verification</option>
                                        <option value="mandator_confirmation" {{ old('verification_method', $dataRemovalRequest->verification_method) == 'mandator_confirmation' ? 'selected' : '' }}>Mandator Confirmation</option>
                                        <option value="other" {{ old('verification_method', $dataRemovalRequest->verification_method) == 'other' ? 'selected' : '' }}>Other</option>
                                    </select>
                                </div>
                                <div>
                                    ...
                                    <textarea name="verification_notes" id="verification_notes" rows="2"
                                              class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                                              placeholder="Additional notes about verification...">{{ old('verification_notes', $dataRemovalRequest->verification_notes) }}</textarea>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mb-6">
                            <div class="flex items-center">
                                <input type="checkbox" name="notify_third_parties" id="notify_third_parties" value="1"
                                       {{ old('notify_third_parties', $dataRemovalRequest->notify_third_parties) ? 'checked' : '' }}
                                       class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                            ...
                            <div class="mt-4">
                                ...
                                <textarea name="third_party_notification_details" id="third_party_notification_details" rows="3"
                                          class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                                          placeholder="Details about third parties to be notified...">{{ old('third_party_notification_details', $dataRemovalRequest->third_party_notification_details) }}</textarea>
                            </div>
                        </div>
                        ...
                        <div class="mb-6">
                            ...
                            <textarea name="retention_justification" id="retention_justification" rows="3"
                                      class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                                      placeholder="If any data needs to be retained, provide justification...">{{ old('retention_justification', $dataRemovalRequest->retention_justification) }}</textarea>
                        ...
                        <div class="mb-6">
                            ...
                            <input type="text" name="legal_basis_for_retention" id="legal_basis_for_retention"
                                   value="{{ old('legal_basis_for_retention', $dataRemovalRequest->legal_basis_for_retention) }}"
                                   class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                        ...
                        <div class="flex justify-end space-x-3">
                            <a href="{{ route('data-removal-requests.show', $dataRemovalRequest) }}"
    ...
    <script>
        ...
        document.getElementById('customer_id').addEventListener('change', function() {
            if (this.value) {
                document.getElementById('mandator_id').value = '';
            }
        });

        document.getElementById('mandator_id').addEventListener('change', function() {
            if (this.value) {
