...
<head>
    ...
    <title>Add Representative - PrivacyCall</title>
...
<body class="bg-gray-50">
    ...
    <div class="max-w-4xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-6">
            ...
            <h1 class="text-3xl font-bold text-gray-900 mt-2">Add New Representative</h1>
            <p class="text-gray-600 mt-2">Create a new company representative with disclosure subscriptions</p>
        </div>
                        ...
                        @else
                            <div>
                                ...
                                <select name="company_id" id="company_id" required
                                        class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 @error('company_id') border-red-500 @enderror">
                                    <option value="">Select a company</option>
                                    @foreach($companies as $companyOption)
                                        <option value="{{ $companyOption->id }}" {{ old('company_id') == $companyOption->id ? 'selected' : '' }}>
                                            {{ $companyOption->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        @foreach($disclosureTypes as $value => $label)
                            <div class="flex items-center">
                                <input type="checkbox" name="disclosure_subscriptions[]" id="disclosure_{{ $value }}"
                                       value="{{ $value }}"
                                       {{ in_array($value, old('disclosure_subscriptions', [])) ? 'checked' : '' }}
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="notes" id="notes" rows="3"
                                      ...
                                      placeholder="Additional notes about this representative...">{{ old('notes') }}</textarea>
                            @error('notes')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
