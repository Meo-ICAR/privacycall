...
@section('title', 'Rights Requests Dashboard')
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
                <h2 class="text-2xl font-bold text-gray-900 mb-6">Data Subject Rights Requests Dashboard</h2>
                ...
                <div class="bg-white shadow overflow-hidden sm:rounded-md">
                    <div class="px-4 py-5 sm:px-6">
                        ...
                        <p class="mt-1 max-w-2xl text-sm text-gray-500">Latest data subject rights requests</p>
                    </div>
                    <ul class="divide-y divide-gray-200">
                        @forelse($recentRequests ?? [] as $request)
                        <li class="px-4 py-4 sm:px-6 flex justify-between items-center">
                            <div>
                                <div class="text-sm font-medium text-gray-900">{{ $request->data_subject_name ?? 'N/A' }}</div>
                                <div class="text-sm text-gray-500">Right: {{ $request->right_type ?? 'N/A' }}</div>
                            </div>
                            <div class="flex items-center">
                                ...
                                <span class="ml-2 text-sm text-gray-500">{{ $request->created_at ? $request->created_at->diffForHumans() : 'Recently' }}</span>
                            </div>
                        </li>
                        @empty
                        <li class="px-4 py-8 sm:px-6 text-center">
                            <div class="text-gray-500">
                                ...
                                <h3 class="mt-2 text-sm font-medium text-gray-900">No recent requests</h3>
                                <p class="mt-1 text-sm text-gray-500">Get started by creating your first request.</p>
                            </div>
                        </li>
                        @endforelse
                    </ul>
